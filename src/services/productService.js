...
class ProductService {
      ...
      .then(
        resp => {
          ...
          if (data.data) {
            ...
            errCode ? errorHandler(errCode) : $.alert("操作失败");
          }
        },
        resp => {
  ...
  quickWithdraw(productId, amount, taAccountId, vue) {
    ...
    _self.$http
      .post(
        config.ajax._post.quickWithdraw,
        {
          productId: productId,
          amount: amount,
          taAccountId: taAccountId
      ...
      .then(
        resp => {
          ...
          if (data.data) {
            ...
            errCode ? errorHandler(errCode) : $.alert("赎回失败");
          }
        },
        resp => {
  ...
  getTotalAsset(productId, vue) {
    return new Promise((resolve, reject) => {
      let [data, errCode, _self] = ["", "", vue];
      _self.$http
        .get(config.ajax._get.totalAsset + "?productId=" + productId)
        .then(
          resp => {
            data = resp.body;
            errCode = data.errCode;
  ...
  getRemain(taAcctId, productId, vue) {
    return new Promise((resolve, reject) => {
      ...
      _self.$http
        .post(
          config.ajax._post.totalAsset,
          { taAcctId: taAcctId, productId: productId },
          { emulateJSON: true }
        )
        .then(
          resp => {
  ...
  getMaxRemain(prodCode, vue) {
    return new Promise((resolve, reject) => {
      let [data, errCode, _self] = ["", "", vue];
      _self.$http.get(config.ajax._get.remain + "?prodCode=" + prodCode).then(
        resp => {
          data = resp.body;
          errCode = data.errCode;
          if (!errCode) {
  ...
  getTaInfo(issueId, vue) {
    return new Promise((resolve, reject) => {
      let [data, errCode, _self] = ["", "", vue];
      _self.$http.get(config.ajax._get.taInfo + "?issueId=" + issueId).then(
        resp => {
          data = resp.body;
          errCode = data.errCode;
          if (!errCode) {
  ...
  confirmAcct(payload, vue) {
    return new Promise((resolve, reject) => {
      ...
      _self.isOTF
        ? (url = config.ajax._post.lofConfirmAcct)
        : (url = config.ajax._post.etfConfirmAcct);
      _self.$http.post(url, payload).then(
        resp => {
          data = resp.body;
          errCode = data.errCode;
          if (data.data) {
  ...
  regular(parm, vue) {
    let [data, errCode, _self] = ["", "", vue];
    _self.$http
      .post(config.ajax._post.taSubscribe, parm, {
        emulateJSON: true
      })
      .then(
        resp => {
  ...
  regularAccount(vue) {
    let [data, errCode, _self] = ["", "", vue];
    return new Promise((resolve, reject) => {
      _self.$http.get(config.ajax._get.allTaAcct).then(
        resp => {
          data = resp.body;
          errCode = data.errCode;
          if (data.data) {
            ...
            errCode
              ? errorHandler(errCode)
              : window.layer.msg("获取账户失败", { time: 1300 });
          }
        },
        resp => {
