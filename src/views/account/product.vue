<template>
    <div id="product">
        <div class="top-nav">
            <div class="tab"><span class="active" @click="changeActive($event)">活期产品</span><span @click="changeActive($event)">定期产品</span></div>
        </div>
        <template v-if="!current">
                ...
                <div class="content">
                    <template v-for="item in filterMarketType(1)">
                        <div class="item">
                            <div class="top">
                                <div class="name">
                                    <a :href="item.linkUrl"><span>{{ item.name }}</span></a>
                                </div>
                            </div>
                            <div class="middle">
                                <div class="block">
                                    <span class="intro">七日年化收益率</span>
                                    <span class="number">{{ item.seventhYearYield | toPercent }}<em>%</em></span>
                                </div>
                                <div class="block">
                                    <span class="intro">万份收益：<em>{{ item.yieldPerMillion | keepDecimal }}</em>元</span>
                                    <span class="intro">爱 心 值: <em>{{item.pointRate | tonIteger}}/亿</em></span>
                                    <span class="intro">更新日期: <em>{{ item.uptTime | formatDate('yyyy.MM.dd') }}</em></span>
                                </div>
                                <div class="block">
            ...
            <div class="section">
                ...
                <div class="content">
                    <template v-for="item in filterMarketType(2)">
                        <div class="item">
                            <div class="top">
                                <div class="name">
                                    <a :href="item.linkUrl"><span>{{ item.name }}</span></a>
        ...
        <template v-if="current">
            <div class="section">
                <div class="content ">
                    <template v-for="item in filterMarketType(null)">
                        <div class="item item-current" :class='{"current-end":item.endFlag===true}'>
                            <div class="top top-current ">
                                <div class="name"><span>{{ item.name }}<br v-if="item.name.length>10 && item.name.length<18">
                                        <em :class="{'site':item.name.length>18}"> {{item.pointRate | tonIteger}}/亿元（日终存量）</em></span>
                                </div>
                            </div>
                            <div class="middle current">
                                <div class="block">
                                    <span class="intro">预期报价收益率</span>
                                    <span class="number">{{ item.yieldPerMillion | toPercent }}<em>%</em></span>
                                </div>
                                <div class="block">
                            ...
                            <div class="bottom">
                                <div class="block">
                                    <span>资金追加日:<em>{{ item.fundAccessDate | formatDate('yyyy.MM.dd') }}</em></span>
                                    <span>产品计息日:<em>{{ item.yieldStartDate | formatDate('yyyy.MM.dd') }}</em></span>
                                </div>
                                <div class="block">
                                    <span>产品到期日:<em>{{ item.yieldEndDate | formatDate('yyyy.MM.dd') }}</em></span>
                                    <span>资金到账日:<em>{{ item.fundExitDate | formatDate('yyyy.MM.dd') }}</em></span>
                                </div>
                            </div>
                        </div>
                    </template>
